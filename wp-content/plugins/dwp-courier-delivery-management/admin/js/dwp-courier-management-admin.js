jQuery(function() {
    ...
    jQuery(document).on("click", "#verification_document", function() {
        ...
        var verifactionImage = wp.media({
            title: dwp_delivery_ajax.v_msg,
            multiple: false
        }).open().on("select", function(e) {

            var uploaded_verification_image = verifactionImage.state().get("selection").first();

            var verification_image_data = uploaded_verification_image.toJSON();

            jQuery("#merchant-verification-document").attr("src", verification_image_data.url);
    ...
    jQuery(document).on("click", "#merchant_shop_icon", function() {
        ...
        var shopicon = wp.media({
            ...
            jQuery("#merchant-shop-image").attr("src", merchant_shop_image_data.url);
    ...
    jQuery(document).on("click", "#merchant-delete-btn", function() {
        ...
        if (conf) {
            var postdata = "action=dwp_form_action&param=delete_merchant_record&merchant_id=" + merchant_id;
    ...
    jQuery(document).on("click", ".btn-delete-delivery-request", function() {
        ...
        if (conf) {
            var postdata = "action=dwp_form_action&param=delete_delivery_record&delivery_request_id=" + delivery_request_id;
